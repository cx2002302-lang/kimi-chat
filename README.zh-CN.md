...
- 布局复刻会话窗口：左侧话题栏 + 主聊天区——**860px 居中内容栏**（kimi 原生观感），助手卡片拉满栏宽、与用户气泡栏内右缘对齐，任意窗口尺寸/缩放都对齐；底部 composer 同栏宽
- **内容概括大纲**（原生交互）：平时右缘只有一条 26px 细指示条（虚线 + 提问小刻度 + 蓝色当前位置滑块）；鼠标靠近即滑出文字大纲面板——每条提问一行右对齐总结，当前项主题色高亮 + 竖色条，点击跳转、跟随滚动，移开自动收回，窄窗口自动隐藏
