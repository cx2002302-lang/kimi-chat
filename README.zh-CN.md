...
- kimi web「设置」内置**插件设置页签**（答复样式：dsh 美学系统 12 风格、配色模式、思考折叠、生图/TTS provider、系统提示词）；思考流折叠收纳，不再刷屏
- **答复风格（新）**——详略 / 图文占比 / 语气 / Emoji / 结构 / 举例 六维配置，编译进系统提示词；三层设置、字段级合并，优先级**聊天 > 文件夹（沿祖先链）> 默认**，入口：设置页 / 文件夹 ⋯ 菜单 / 聊天头部 🎨 按钮
...
16 套共 287 项全绿：unit 32 + service 25 + heal 10 + 截断续写 5 + 图片代理 8 + 浏览器回归 14 + 安全 13 + 文件夹选择器 26 + v0.7.2 布局 20 + v0.7.3 原生观感 12 + v0.7.4 文件夹与导航 51 + v0.8.1 样式与进度条 36 + v0.8.2 按钮与思考折叠 12 + 风格级联 9 + v0.9.0 风格 UI 与配色模式 10 + v0.9.2 卡片深色适配 5。
