...
- 布局复刻会话窗口：左侧话题栏 + 主聊天区——**流动栏（最大 1500px）**内助手卡片拉满栏宽、与用户右气泡右缘对齐，任意窗口尺寸/缩放都对齐；底部 composer 同栏宽
- **内容概括大纲**：复刻 kimi 原生的文字导航，dock 在聊天区右缘——每条提问一行右对齐总结、超长省略号，当前项主题色高亮 + 左侧竖色条，点击跳转、跟随滚动，窄窗口自动隐藏；独立成栏，不遮挡消息、不压滚动条
