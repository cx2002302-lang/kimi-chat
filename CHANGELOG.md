...
## v0.9.2（2026-09-23）—— 卡片深色适配（深色风格下不再刺眼）

- **修复深色环境下模型卡片白底刺眼**：卡片是模型自带样式的 HTML，常硬编码浅色底
  （#fff/white/#fafafa 等）。现在深色环境下渲染层做两级适配：①卡片根容器强制
  `background:var(--vcp-base)` / `color:var(--vcp-text-primary)`——**继承所选答复风格的
  深色色板**（未选风格时中性深色兜底）；②内联浅色块（含 rgb(255,255,255) 写法）批量
  换成 `var(--vcp-surface)` 深色面板。卡片自带 `data-vcp-mode="dark"` 时尊重原设计不动。
  浅色环境完全不变（实测白卡保持白底）。
- **提示词侧同步**：聊天请求携带当前配色（`scheme`），深色环境下系统提示词追加
  「VCP 卡片请使用深色底（正文对比度 ≥4.5:1）」——新生成的卡片从源头就是深色。
- **修复渲染器竞态降级**：卡片渲染失败时补日志（此前静默降级成代码块无从排查）；
  本次即通过该日志定位并修复了一个跨作用域调用导致的降级 bug。
- 新增 `tests/v091-browser.cjs`（5 项）；`style-profile` 增补深色提示词 2 项。

## v0.9.1（2026-09-23）—— 配色模式修复 + 代码清扫

- **修复「配色模式」设置从未生效**：浅色/深色/跟随三项存得下、读得出，但界面从没应用过
  （`resolveColorMode` 定义了没人调用）。现在通过宿主属性 `data-kc-scheme` 强制覆盖——
  模块底色、导航条、以及 **12 个答复样式的明/暗色板**全部跟随；「跟随 kimi web」保持原行为。
  实测：宿主浅色下强制深色 = rgb(18,18,18)；宿主深色下强制浅色 = rgb(255,255,255)。
- 清扫：删除失效的配色引擎调用路径（答复样式改由 CSS 提供色板后 `resolveColorMode` 死代码
  移除），旧样式 slug 迁移逻辑入库（editorial → editorial-minimal 等）。

## v0.9.0（2026-09-23）—— 答复样式按 dsh 美学系统重做 + 答复风格三层配置

- **答复样式重做（dsh-raw-html 美学系统 12 风格）**：原「配色引擎随机色板」5 档替换为
  项目美学系统的 12 个命名风格（青瓷数据 / 编辑部红 / 侘寂 / 笺信 / 顶栏装帧 / Y2K 玻璃 /
  暖调极简 / 编辑极简 / 赛博朋克 / 波普色块 / 粗野主义 / 少女手账），色板逐一取自各风格
  文档（明/暗双态内置），不再依赖运行时引擎生成；旧 slug 自动迁移（editorial →
  editorial-minimal 等）。
- **答复风格（新）——六维配置 + 三层级联**：详略 / 图文占比 / 语气 / Emoji / 结构 / 举例
  六个维度，生成对应提示词片段注入系统提示词。配置位置三层：**设置页（默认档）→
  文件夹（含祖先链）→ 本聊天**，字段级合并，优先级聊天 > 文件夹 > 默认；入口：
  设置页「答复风格（默认档）」、文件夹 ⋯ 菜单「🎨 答复风格…」、聊天头部「🎨 风格」，
  支持「跟随上层 / 清除回继承」，保存后下一条消息生效。
- **修复文件夹创建接口不透传 style_profile**（仅 PATCH 支持）；风格弹窗按钮与设置表单
  的保存动作解耦（独立 data-sdlg 属性），避免冒泡误触发设置保存；模块关闭时同步收起
  风格弹窗。
- 新增 `tests/style-profile.test.mjs`（级联 7 项）+ `tests/v090-browser.cjs`（三层 UI 冒烟 8 项）；
  v081 预设体检适配新风格（data-kcstyle 断言）。

## v0.8.2（2026-09-23）—— 追问按钮修复 + 思考折叠原生风 + 复制回退

- **修复「卡片里的按钮/直贴按钮点不动」**：`onclick="input('…')"` 的全局桥（原生 dsh 约定）
  在本插件里**从未被定义**——所有可交互按钮点击都是静默 ReferenceError。现已补上全局桥：
  点击填充输入框并聚焦。同时**裸 button 识别**：模型把追问按钮直贴到 ` ```vcp ` 围栏外时
  （此前显示为标签文本），自动还原为真实可点按钮（`.kc-tap` 样式，事件委托安全绑定）。
- **思考折叠改原生会话风**：头行 `💭 思考过程 · Ns · N 字`（时长 + 字数，对齐原生
  「思考过程 · 19s」），正文改为**内嵌滚动窗**（max-height 180px、细滚动条），不再撑开
  消息流；思考全文与时长由服务端持久化（上限 2 万字），**刷新后折叠依旧在**。
- **修复 ⧉ 复制按钮不生效**：原来只走 `navigator.clipboard`——局域网 http 非安全上下文里
  该 API 不存在，点击无反应；现改走带 execCommand 回退的复制通道（同 rail 的复制 ID）。
- 新增 `tests/v082-browser.cjs`（12 项：裸按钮桥 4 + 卡片按钮桥 2 + 复制回退 2 + 思考折叠 4）。

## v0.8.1（2026-09-23）—— 答复样式全档适配 + 裸 HTML 转 MD + 思考进度条

- **修复「原生」样式存不进去**：`/api/config` 的 PUT 用「跳过空字符串」规则合并字段，
  导致 `card_style: ''`（原生模式）永远保存失败——设置里选「原生」看似不生效。已放行
  空串为合法值。
- **答复样式 6 档逐一程序化体检**（原生 + Editorial/Chiaroscuro/Fauvism/Cyberpunk/
  Wabi-sabi × 明/暗）：配色变量注入与正文对比度（≥3.5:1，实测 16~21:1）全部通过。
- **裸 HTML → Markdown**：模型未走 ` ```vcp ` 围栏直贴 HTML 时（违反输出纪律的兜底），
  不再显示成标签汤文本——表格/标题/列表/加粗/斜体/引用/分割线转成对应 Markdown 排版，
  其余标签剥离（触发门槛为块级元素，不误伤正文里的字面量）。
- **思考进度条**：流式期间「💭 思考中…（N 字）」改为**对数渐进进度条**——思考越深入
  越满（对数曲线，前期快后期缓、永不虚满），思考结束标签切为「已思考」；收尾后仍折叠
  为「思考过程（N 字）」明细。
- **修复 opencode-go 后端 400**：新版 CLI 默认模型（opencode-go/deepseek-v4.1-flash）要求
  带 provider 的 `custom_headers`（`x-opencode-session`）才能路由；注册表解析补齐该字段，
  聊天与自动起名请求均透传，聊天恢复。
- 新增 `tests/v081-browser.cjs`（36 项：裸 HTML 转换 6 + 预设体检 24 + 进度条 6）。

## v0.8.0（2026-09-23）—— 卡片外链图片修复 + 渲染防连坐

- **修复「卡片里的 HTML 渲染不出来」（外链图部分）**：kimi web 的 CSP 拦截一切外链
  `<img>`，卡片引用网络图片会空白。新增同源图片代理 `GET /api/img-proxy?url=…`：
  前端把卡片/正文里的外链图自动改写走本服务转发（`access_token` 认证、仅放行
  `image/*` content-type 防 HTML 注入、拒云元数据地址、12MB 上限、15s 超时）。
  端到端实测：外链图经代理真实加载、零 CSP 违规。
- **渲染防连坐**：打开话题时逐条消息渲染加异常保护——单条渲染失败自动降级为纯文本，
  不再连坐吞掉后面的消息（「整轮消失」类问题的保险丝）；流式结束后的最终渲染失败
  也兜底为纯文本全文。
- 新增 `tests/imgproxy.test.mjs`（8 项：转发/认证/415/400/502）。注：话题数据与
  渲染器经多轮实测未发现「吃轮」——若仍见消息缺失，多为浏览器旧缓存，强刷即可。

## v0.7.9（2026-09-23）—— 根治「挤牙膏」：回复不再被截断

- **根因**：`max_tokens: 8192` 对推理模型（GLM 等）是**思考 + 正文共用预算**——新话题首轮
  思考最长，预算被吃光后正文只剩三百来字就被 `finish_reason=length` 拦腰截断（实测同一
  问题在 8192 下正文 373 字中断、16384 下 7324 字完整收尾）。
- **修复**：① 输出上限提到 **16384**；② 新增**长度截断自动续写**——流式返回
  `finish_reason=length` 时自动带上已产出内容发起续写（最多 3 轮，续写指令要求从中断处
  直接接续、不重复），客户端无感拿到合并后的完整正文；③ 每轮 finish 原因写入
  `watch.log`（`chat-finish round=… reason=… chars=…`）便于观测。
- 新增 `tests/chat-continue.test.mjs`（本地假上游模拟 length 截断→续写→合并，5 项）；
  v073 测试对「默认无卡片框」断言改为尊重用户覆盖的 `card_style` 配置（跳过并注明）。
- 实测：五行问题 5951 字一次说完、自然收尾 + 交互按钮，`round=0 reason=stop`。

## v0.7.8（2026-09-22）—— 拖拽根治 + 文件夹内新建话题

- **修复「拖进文件夹后就拖不动」**：根因有两层——① `dragstart` 处理器里同步显示
  「移到未分组」悬放条（display:none→block 布局变化）会当场掐死原生 DnD 会话
  （dragend 立即触发、之后无法再拖）；② drop 后立刻重渲染会把拖拽源移出 DOM，
  dragend 丢失、会话卡死。修复：悬放条改为**悬浮在列表顶部的绝对定位覆盖层**
  （出现/消失零布局变化），其显示延后到 dragstart 返回之后，drop 的移动请求延后到
  DnD 会话收尾再执行。
- **投放区扩大**：悬停/投放到文件夹的子话题上等同于投到该文件夹（整块文件夹区域
  都是有效投放区，不再只有标题行那一细条）。
- **文件夹菜单新增「💬 新建话题」**：直接在该文件夹下创建话题并打开（服务端
  `POST /api/topics` 原生支持 `folder_id`）。
- 测试：v074 新增「文件夹内话题再拖到另一文件夹」「文件夹菜单新建话题」用例并加
  夹具自清理与拖拽前滚动可见（51 项）；全量回归通过。

## v0.7.7（2026-09-22）—— 导航条挂载位置对齐原生

- **修复导航条挂载错位**：原挂在主区最右缘（滚动条右侧），悬停展开时标签向右生长直接
  出屏。重新采样原生几何（1920×1080：原生 nav 左缘=居中消息栏右缘 1489，展开左缘不动、
  宽 13→233 向右生长）后，把挂载点改为**居中 860px 消息栏的右缘**（零宽锚点
  `left: calc(50% + 430px)`），展开标签长进栏右侧空白画布——与原生完全同构，全程在屏内。
- 新增挂载位置回归断言（nav 左缘 ≈ 栏中线+430、展开左缘不动且不出右缘）。

## v0.7.6（2026-09-22）—— 交互细节修复

- **列表底部话题的 ⋯ 菜单不再出屏**：原定位按估算高度（260px）夹取，菜单项多时底部
  （清空/删除）落到屏幕外点不到；改为显示后实测高度，下方放不下就整体向上弹出，
  保证完整可见可选（话题菜单 / 文件夹菜单 / 「移动到…」三处统一）。
- **左下角设置入口整体可点**：原只有 ⚙️ 小图标是按钮，旁边文本点不到；现整行是一个
  button（`⚙️ 设置 · kimi-chat v…`），文字区域同样打开设置。
